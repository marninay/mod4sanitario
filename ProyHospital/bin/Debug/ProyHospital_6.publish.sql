...
CREATE TABLE [dbo].[HistoriaClinica] (
    ...
    PRIMARY KEY CLUSTERED ([codHist] ASC)
);
...
CREATE TABLE [dbo].[Internacion] (
    ...
    PRIMARY KEY CLUSTERED ([idinternacion] ASC)
);
...
GO
PRINT N'Creando Clave externa restricción sin nombre en [dbo].[HistoriaClinica]...';


GO
ALTER TABLE [dbo].[HistoriaClinica] WITH NOCHECK
    ADD FOREIGN KEY ([DNIPaciente]) REFERENCES [dbo].[Paciente] ([DNI]);


GO
PRINT N'Creando Clave externa restricción sin nombre en [dbo].[Internacion]...';


GO
ALTER TABLE [dbo].[Internacion] WITH NOCHECK
    ADD FOREIGN KEY ([idVisita]) REFERENCES [dbo].[VisitaMedica] ([idVisita]);
...
ALTER TABLE [dbo].[MedicoServicio] WITH NOCHECK
    ADD FOREIGN KEY ([DNI]) REFERENCES [dbo].[Medico] ([DNI]);
...
GO
ALTER TABLE [dbo].[MedicoServicio] WITH NOCHECK
    ADD FOREIGN KEY ([codHospital]) REFERENCES [dbo].[Hospital] ([codHospital]);


GO
PRINT N'Creando Clave externa restricción sin nombre en [dbo].[VisitaMedica]...';
...
GO
ALTER TABLE [dbo].[VisitaMedica] WITH NOCHECK
    ADD FOREIGN KEY ([DNIMedico]) REFERENCES [dbo].[Medico] ([DNI]);
...
GO
ALTER TABLE [dbo].[VisitaMedica] WITH NOCHECK
    ADD FOREIGN KEY ([codHist]) REFERENCES [dbo].[HistoriaClinica] ([codHist]);
...
DECLARE tableconstraintnames CURSOR LOCAL FORWARD_ONLY
    FOR SELECT SCHEMA_NAME([schema_id]),
               ...
               0
        FROM   [sys].[objects]
        WHERE  [parent_object_id] IN (OBJECT_ID(N'dbo.HistoriaClinica'), OBJECT_ID(N'dbo.Internacion'), OBJECT_ID(N'dbo.Medico'), OBJECT_ID(N'dbo.MedicoServicio'), OBJECT_ID(N'dbo.VisitaMedica'))
               AND [type] IN (N'F', N'C')
